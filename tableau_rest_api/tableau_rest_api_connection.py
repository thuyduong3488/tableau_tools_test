...
from tableau_common.tableau_base import *
from tableau_documents.tableau_file import TableauFile
from tableau_common.tableau_exceptions import *
...
from tableau_rest_api.published_content import Project20, Project21, Workbook, Datasource
import copy
...
class TableauRestApiConnection(TableauBase):
    ...
    def query_workbook_preview_image(self, wb_name_or_luid, proj_name_or_luid=None):
        ...
        try:
            ...
            url = self.build_api_url("workbooks/{}/pdf?orientation=Landscape".format(wb_luid))
            image = self.send_binary_get_request(url)
            self.end_log_block()
            return image
    ...
    # Do not include file extension
    def save_workbook_preview_image(self, wb_name_or_luid, file, proj_name_or_luid=None):
        """
        :type wb_name_or_luid: unicode
    ...
    def add_user_by_username(self, username=None, site_role='Unlicensed', update_if_exists=False,
    ...
    def update_workbook_now(self, wb_name_or_luid, proj_name_or_luid=None):
        """
        :type wb_name_or_luid: unicode
        :type proj_name_or_luid: unicode
        :rtype: bytes
        ...
        try:
            ...
            request = etree.Element('tsRequest')
            response = self.send_add_request(url, request)
            self.end_log_block()
            return response

        # You might be requesting something that doesn't exist, but unlikely
        except RecoverableHTTPException as e:
            self.log("Attempt to run update workbook results in HTTP error {}, Tableau Code {}".format(e.http_code,
                                                                                                       e.tableau_error_code))
            self.end_log_block()
            raise
